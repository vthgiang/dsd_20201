...
class ObjMonitorActivity extends React.Component {
...
class ObjMonitor extends React.Component {
...
class App extends React.Component{
  ...
  setLogData(fromDate, toDate) {
    let url = null;
    if (fromDate && toDate) {
      url = 'https://it4883logging.herokuapp.com/api/monitor-object?minDate=' + fromDate +'&maxDate=' + toDate +'&username=G3&password=123';
    } else {
      url = 'https://it4883logging.herokuapp.com/api/monitor-object?username=G3&password=123';
    }
    ...
    axios(config)
      .then((response) => {
        ...
        let objMonitorData = response.data.map((objMonitor, index) => ({
          key: index,
        ...
        objMonitorData.forEach((objMonitorData) => {
          for(let key in objMonitorData) {
            if (objMonitorData[key] == null) objMonitorData[key] ='';
          }
        });
        this.setState({ logData: objMonitorData, isLoadedLogData: true });
  ...
  setLogActivityData(fromDate, toDate) {
    let url = null;
    if (fromDate && toDate) {
      url = 'https://it4883logging.herokuapp.com/api/activity/object-observe?minDate=' + fromDate +'&maxDate=' + toDate +'&username=G3&password=123';
    } else {
      url = 'https://it4883logging.herokuapp.com/api/activity/object-observe?username=G3&password=123';
    }
    ...
    axios(config)
      .then((response) => {
        let objMonitorActivityData = response.data.map((objMonitor, index) => ({
          ...
          description:objMonitor.description,
          timestamp:objMonitor.timestamp,
         state:objMonitor.state
          
        }));
        objMonitorActivityData.forEach((objMonitorData) => {
          for(let key in objMonitorData) {
            if (objMonitorData[key] == null) objMonitorData[key] ='';
          }
        });
        this.setState({ logActivityData: objMonitorActivityData, isLoadedLogActivityData: true });
      })
      .catch(function (error) {
        console.log(error);
      });
...
<h1>
  ...
  <Radio.Group buttonStyle="solid" onChange={(e) => {this.onTableShowChange(e.target.value)}} style={{marginBottom:'20px'}}>
            ...
            <div style={{ display: this.state.tableShow === 'log' ? "block" : "none" }}>
              <ObjMonitor data={this.state.logData} loading={!this.state.isLoadedLogData}/>
            </div>
            <div style={{ display: this.state.tableShow === 'logActivity' ? "block" : "none" }}>
              <ObjMonitorActivity data={this.state.logActivityData} loading={!this.state.isLoadedLogActivityData}/>
            </div>
